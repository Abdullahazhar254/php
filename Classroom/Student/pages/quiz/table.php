                ...
                <?php
                    ...
                    if($result1){
                        if(mysqli_num_rows($result1) > 0){
                            ...
                            ?>
                <a href="Submit.php?id=<?php echo $row['q_id'] ?>" >
                <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                    <div class="card">
                        <div class="body  bg-light-green">
                            <h3><?php echo $row['q_name'] ?></h3>
                            <h4>Total Question : <?php echo $row['q_question'] ?></h4>
                            <h4>Start Date : <?php echo $row['q_startdate'] ?></h4>
                            <h4>End Date : <?php echo $row['q_enddate'] ?></h4>
                            <h4>Note : <?php if($row['q_enddate']<=date("Y-m-d")){
                                echo "Your will get half marks foreach question due to Late submission"; 
                            }else {
                                echo "Submit before end date otherwise you will get half marks foreach question";
                            }
                                ?></h4>
                            <h4>Click to start Quiz.</h4>
                        </div>
                    </div>
                </div>
                </a>
